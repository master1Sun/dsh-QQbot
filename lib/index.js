...
async function readJson(path4) {
  try {
    return JSON.parse(await readFile2(path4, "utf8"));
  } catch {
    return null;
  }
}
function readStoreJson(path4) {
  return readJson(path4);
}
async function writeJson(path4, value, { privateFile = false } = {}) {
  ...
  await rename(tmp, path4);
}
function writeStoreJson(path4, value) {
  return writeJson(path4, value);
}
...
function installReplyPump(ctx, { bots, outbox, quota, logger }) {
  ...
  const send = async (bot, sessionId, text) => {
    ...
    const quoteMsgId = record.target.scope === "group" && config.quoteReply !== "off" && (config.quoteReply === "all" || record.quoteMention) ? record.selfIdx || void 0 : void 0;
...
// src/host/memory.ts
init_store_file();
import { mkdir as mkdir5, readFile as readFile6, rm as rm3, writeFile as writeFile4 } from "node:fs/promises";
...
var ChatMemoryStore = class {
  ...
  async load(chatKey) {
    ...
    let entries = [];
    try {
      const raw = JSON.parse(await readFile6(memoryPath(chatKey), "utf8"));
  ...
  async #save(chatKey, entries) {
    try {
      await mkdir5(join4(pluginDataDir(), "memory"), { recursive: true });
      await writeFile4(memoryPath(chatKey), JSON.stringify(entries, null, 2), "utf8");
      this.#cache.set(chatKey, entries);
    } catch (error) {
