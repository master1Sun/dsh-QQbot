...
function formatCommandResult(result, limit = COMMAND_OUTPUT_LIMIT) {
  ...
  const body = result.ok ? result.stdout.trim() || (result.stderr.trim() ? `（无标准输出）stderr：
...
function permissionsDir(appId) {
  return join4(pluginDataDir(), "permissions", safeKey(appId ?? ""));
}
function defaultPath(appId) {
  return join4(permissionsDir(appId), `${DEFAULT_KEY}.md`);
}
...
async function runCommand2(text, { scope, openid, sender, msgId }, ctx) {
...
var RefIndex = class {
  ...
  constructor({ logger, appId, max = DEFAULT_MAX }) {
    ...
    this.#path = join5(pluginDataDir(), `ref-index-${safeAppId}.jsonl`);
  }
  /** 落盘路径（设置界面展示用）。 */
  get path() {
    return this.#path;
  ...
  return {
    ...
    async run(delivery) {
      ...
      if (eventType === "GROUP_MESSAGE_CREATE") {
        ...
        const command = await runCommand2(
          content,
      ...
      if (!scheduled) {
        const command = await runCommand2(content, { scope: target.scope, openid: target.openid, sender, msgId }, commandCtx);
        if (command.handled) return null;
      }
...
var SelfOpenidStore = class {
  ...
  async save(appId, map) {
    try {
      await mkdir6(join6(pluginDataDir(), "self-openids"), { recursive: true });
      await writeFile6(filePath(appId), JSON.stringify(Object.fromEntries(map), null, 2), "utf8");
    } catch (error) {
...
var StatsStore = class {
  ...
  async save(appId, counters) {
    try {
      await mkdir7(join7(pluginDataDir(), "stats"), { recursive: true });
      await writeFile7(
        statsPath(appId),
...
${entry.genPrompt ?? ""}` }],
      ...
      const dir = join8(pluginDataDir(), "scripts");
      await mkdir8(dir, { recursive: true });
      const absPath = join8(dir, `${entry.id}-${parsed.filename}`);
      await writeFile8(absPath, scriptFileBuffer(parsed.filename, parsed.code));
...
init_store_file();
import { mkdir as mkdir9, readFile as readFile9, rm as rm5, writeFile as writeFile9 } from "node:fs/promises";
...
function memoryPath(chatKey) {
  return join9(pluginDataDir(), "memory", `${safeName(chatKey)}.md`);
}
...
var ChatMemoryStore = class {
  ...
  async #save(chatKey, entries) {
    try {
      await mkdir9(join9(pluginDataDir(), "memory"), { recursive: true });
      const lines = ["# QQ 聊天长期记忆", ""];
      for (const e of entries) lines.push(`- ${e.text}`);
      await writeFile9(memoryPath(chatKey), `${lines.join("\n")}
`, "utf8");
      this.#cache.set(chatKey, entries);
    } catch (error) {
...
var OUTBOX_PATH = () => join10(pluginDataDir(), "outbox.json");
var OUTBOX_CAP = 200;
var MAX_ATTEMPTS = 5;
var Outbox = class {
  #items = [];
...
var QUOTA_PATH = () => join11(pluginDataDir(), "quota.json");
