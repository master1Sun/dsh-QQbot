...
function resolveConfig({ entry = {}, stored = {}, credentials = {} } = {}) {
  ...
  return {
    ...
    voiceTranscription: oneOf(pick2("voiceTranscription"), ["off", "note", "download", "asr", "stt"], "off"),
...
function mediaDir() {
  return join6(pluginDataDir(), "media");
}
...
async function synthesizeSpeech(text, config, logger) {
  ...
  try {
    ...
    const path5 = join6(dir, `tts-${hash}.wav`);
...
function filePath(appId) {
  return join7(pluginDataDir(), "self-openids", `${appId.replace(/[^A-Za-z0-9_-]/g, "_")}.json`);
}
var SelfOpenidStore = class {
  ...
  async save(appId, map) {
    try {
      await mkdir7(join7(pluginDataDir(), "self-openids"), { recursive: true });
...
function statsPath(appId) {
  return join8(pluginDataDir(), "stats", `${appId.replace(/[^A-Za-z0-9_-]/g, "_")}.json`);
}
var StatsStore = class {
  ...
  async save(appId, counters) {
    try {
      await mkdir8(join8(pluginDataDir(), "stats"), { recursive: true });
      await writeFile8(
        statsPath(appId),
