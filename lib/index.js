...
async function readJson(path3) {
  try {
    return JSON.parse(await readFile2(path3, "utf8"));
  } catch {
    return null;
  }
}
function readStoreJson(path3) {
  return readJson(path3);
}
async function writeJson(path3, value, { privateFile = false } = {}) {
...
function writeStoreJson(path3, value) {
  return writeJson(path3, value);
}
...
function resolveConfig({ entry = {}, stored = {}, credentials = {} } = {}) {
  ...
  return {
    ...
    voiceTranscription: oneOf(pick2("voiceTranscription"), ["off", "note", "download", "asr"], "note"),
    asrEndpoint: str(pick2("asrEndpoint")),
    welcomeEnabled: boolOr(pick2("welcomeEnabled"), false),
    welcomeMessage: str(pick2("welcomeMessage")),
    reactionRecall: boolOr(pick2("reactionRecall"), false),
...
function createAdminService(ctx) {
  ...
  const workspaceList = async () => {
    const root = path2.join(homedir3(), ".dsh", "file");
    const workspaces = [];
    try {
      ...
      for (const dirent of dirents) {
        ...
        workspaces.push({ path: path2.join(root, dirent.name), name: dirent.name });
      }
    } catch {
    }
  ...
  const workspaceBrowse = async (payload) => {
    ...
    const target = path2.resolve(requested);
    let info;
    try {
    ...
    try {
      ...
      for (const dirent of dirents) {
        if (!dirent.isDirectory()) continue;
        if (dirent.name.startsWith(".")) continue;
        dirs.push({ path: path2.join(target, dirent.name), name: dirent.name });
      }
    } catch (error) {
    ...
    return {
      ...
      data: {
        ...
        isDefault: target === path2.resolve(root),
        dirs
      }
    };
  };
  ...
  const scheduleList = async (payload) => {
    ...
    const mine = scope && openid ? all.filter((e) => e.scope === scope && e.openid === openid) : all;
...
var MARKDOWN_REJECTION_CODES = /* @__PURE__ */ new Set([40034090, 40034090, 304003, 304024, 304042, 500]);
var QqApiClient = class {
  ...
  async #send(target, payload) {
    ...
    try {
      const body = await res.json();
      return typeof body?.id === "string" && body.id ? body.id : void 0;
    } catch {
      return void 0;
    }
  }
  ...
  async sendMarkdown(target, content, { msgId, msgSeq, quoteMsgId } = {}) {
    const text = content.trim();
    if (!text || !target.openid) return void 0;
  ...
  async sendReply(target, content, {
    ...
    const quote = useRef ? quoteMsgId : void 0;
    if (markdown) {
      try {
        const id2 = await this.sendMarkdown(target, text, { msgId, msgSeq, quoteMsgId: quote });
        return { mode: "markdown", id: id2, nativeQuote: Boolean(quote) };
      } catch (error) {
        const code = error.code;
        if (!MARKDOWN_REJECTION_CODES.has(Number(code)) && code !== void 0) {
          if (quote && code !== void 0) {
            this.#disableNativeQuote(error);
            const id2 = await this.sendMarkdown(target, text, { msgId, msgSeq });
            return { mode: "markdown", id: id2, nativeQuote: false };
          }
          throw error;
        }
    ...
    if (quote) {
      try {
        const id2 = await this.sendText(target, text, { msgId, msgSeq, quoteMsgId: quote });
        return { mode: "text", id: id2, nativeQuote: true };
      } catch (error) {
        const code = error.code;
...
// src/host/schedule.ts
init_store_file();
import { randomUUID } from "node:crypto";
var MAX_SCHEDULES_PER_CHAT = 5;
...
var ScheduleStore = class {
  ...
  async add(input) {
    ...
    if (content.length > 2e3) return { ok: false, error: "消息内容过长（上限 2000 字）" };
    ...
    if (type === "daily") {
      if (!/^\d{1,2}:\d{2}$/.test(input.time ?? "") || !nextDailyRun(input.time, /* @__PURE__ */ new Date())) {
    ...
    const entry = {
      ...
      openid: input.openid,
      type,
      ...type === "daily" ? { time: input.time, minutes: void 0 } : { minutes: Number(input.minutes), time: void 0 },
      mode: input.mode === "ai" ? "ai" : "text",
      content,
      enabled: true,
...
function nextDailyRun(time, now) {
  ...
  switch (cmd) {
    ...
    case "/定时消息": {
      ...
      if (!sub || sub === "查看" || sub === "list") {
        ...
        await reply(mine.length === 0 ? `当前聊天还没有定时消息。用法：/定时 每天 09:00 内容 或 /定时 间隔 30 内容（每聊天最多 ${MAX_SCHEDULES_PER_CHAT} 条）。` : ["当前定时消息：", ...mine.map(describe)].join("\n"));
        return { handled: true };
      }
    ...
    case "/memory": {
      ...
      await reply(entries.length === 0 ? "本聊天还没有长期记忆。对话中让我「记住某事」即可自动写入。" : ["本聊天的长期记忆：", ...entries.map((e, i) => `${i + 1}. ${e.text}`)].join("\n"));
      return { handled: true };
    }
    case "/清空记忆":
    case "/forget": {
...
function formatOutboundQuote(quote, { markdown, maxChars }) {
  const body = quoteBody(quote?.content ?? "", maxChars);
  if (!body) return "";
  const name2 = displayName(quote);
  if (markdown) {
    return `> **${name2}**：${body}
...
var RefIndex = class {
  ...
  constructor({ logger, appId, max = DEFAULT_MAX }) {
    ...
    this.#path = join2(pluginDataDir(), `ref-index-${safeAppId}.jsonl`);
  }
  /** 落盘路径（设置界面展示用）。 */
  get path() {
    return this.#path;
...
`, "utf8");
  ...
  async #compact() {
    ...
    try {
      ...
      await writeFile2(this.#path, text ? `${text}
` : "", "utf8");
    } catch (error) {
...
async function attachmentContextOf(bot, payload, logger) {
...
async function enforceBannedWords(bot, payload, content, logger) {
...
function quoteOf(payload, sender, content) {
  return {
    sender,
    senderName: payload.author?.username ?? "",
    content,
  ...
  return {
    ...
    async run(delivery) {
      ...
      if (eventType === "GROUP_MESSAGE_CREATE") {
        logger.info(
          `[dsh-qqbot] [群消息测试] ${bot.appId} 收到群全量消息 group=${group || "?"} sender=${sender}${payload.author?.username ? `(${payload.author.username})` : ""} atBot=${atBot(payload) ? "是" : "否"} content="${content.slice(0, 100)}"`
        );
...
function enterConversation(args) {
  const {
    bot,
    agents,
    logger,
...
function installReplyPump(ctx, { bots, outbox, quota, logger }) {
  const bus = ctx;
  const send = async (bot, sessionId, text) => {
    const config = bot.config;
    ...
    const quotePrefix = quoteAllowed && !nativeQuote ? formatOutboundQuote(record.quote, { markdown: config.markdownReply, maxChars: config.quoteMaxChars }) : "";
    ...
    for (const [index, chunk] of chunks.entries()) {
      ...
      try {
        const result = await bot.client.sendReply(record.target, body, {
          msgId: record.msgId || void 0,
          msgSeq: seq,
          markdown: usedMarkdown,
          // 原生引用只挂在第一片（后续分片重复挂引用会显得嘈杂）。
          quoteMsgId: nativeQuote && index === 0 ? record.msgId : void 0
        });
...
`;
    try {
      const dir = join3(pluginDataDir(), "archive");
      await mkdir3(dir, { recursive: true });
      await appendFile2(join3(dir, `archive-${month}.jsonl`), line, "utf8");
      this.#warned = false;
    } catch (error) {
      if (!this.#warned) {
        this.#warned = true;
...
function buildQqbotTools({ bots, store, memory, logger }) {
  const requireBot = (exec) => {
...
// src/host/memory.ts
init_store_file();
import { mkdir as mkdir4, readFile as readFile4, rm as rm2, writeFile as writeFile3 } from "node:fs/promises";
...
function memoryPath(chatKey) {
  return join4(pluginDataDir(), "memory", `${safeName(chatKey)}.json`);
}
var ChatMemoryStore = class {
  ...
  async load(chatKey) {
    ...
    let entries = [];
    try {
      const raw = JSON.parse(await readFile4(memoryPath(chatKey), "utf8"));
      if (Array.isArray(raw)) {
        entries = raw.filter((e) => Boolean(e) && typeof e.text === "string").map((e) => ({ ts: typeof e.ts === "string" ? e.ts : "", text: e.text.slice(0, MEMORY_MAX_CHARS) })).slice(-MEMORY_MAX_ENTRIES);
      }
    } catch {
    }
  ...
  async add(chatKey, text) {
    ...
    const entry = { ts: toShanghaiISO(), text: clean };
  ...
  async clear(chatKey) {
    ...
    try {
      await rm2(memoryPath(chatKey), { force: true });
    } catch (error) {
  ...
  async #save(chatKey, entries) {
    try {
      await mkdir4(join4(pluginDataDir(), "memory"), { recursive: true });
      await writeFile3(memoryPath(chatKey), JSON.stringify(entries, null, 2), "utf8");
      this.#cache.set(chatKey, entries);
    } catch (error) {
...
var OUTBOX_PATH = () => join5(pluginDataDir(), "outbox.json");
var OUTBOX_CAP = 200;
var MAX_ATTEMPTS = 5;
var Outbox = class {
  #items = [];
...
var QUOTA_PATH = () => join6(pluginDataDir(), "quota.json");
