...
// src/host/archive.ts
init_store_file();
import { appendFile, mkdir as mkdir2, readdir, readFile as readFile3 } from "node:fs/promises";
...
`;
    try {
      ...
      await appendFile(join2(dir, `archive-${month}.jsonl`), line, "utf8");
      this.#warned = false;
    } catch (error) {
      if (!this.#warned) {
        this.#warned = true;
...
// src/host/updater.ts
import { copyFile, mkdir as mkdir3, readFile as readFile4, rename as rename2, rm as rm2, writeFile as writeFile2 } from "node:fs/promises";
...
async function applyUpdate() {
  ...
  for (const rel of UPDATE_FILES) {
    ...
    await writeFile2(tmp, contents.get(rel), "utf8");
    try {
      await rename2(tmp, dest);
    } catch {
      await writeFile2(dest, contents.get(rel), "utf8");
...
function resolveConfig({ entry = {}, stored = {}, credentials = {} } = {}) {
  ...
  return {
    ...
    localPathWhitelist: boolOr(pick2("localPathWhitelist"), true),
...
function createAdminService(ctx) {
  ...
  const scheduleList = async (payload) => {
    ...
    if (payload.allBots !== true) {
      const appId = typeof payload.appId === "string" && payload.appId ? payload.appId : primaryAppId();
      mine = mine.filter((e) => (e.appId ?? primaryAppId()) === appId);
    }
...
var EN = Object.freeze({
  ...
  "（上次失败：": " (last failed: ",
  "）": ")",
  "当前聊天还没有定时消息。用法：/定时 每天 09:00 内容 或 /定时 间隔 30 内容（每聊天最多 5 条）。": "No scheduled messages in this chat yet. Usage: /定时 daily 09:00 text or /定时 interval 30 text (max 5 per chat).",
  ...
  // ── /广播 ──
  "用法：/广播 <内容>（向本机器人已见过的所有群发送）": "Usage: /广播 <text> (send to all groups this bot has seen)",
  ...
  switch (cmd) {
    ...
    case "/定时消息": {
      ...
      if (!sub || sub === "查看" || sub === "list") {
        ...
        await reply(mine.length === 0 ? T(`当前聊天还没有定时消息。用法：/定时 每天 09:00 内容 或 /定时 间隔 30 内容（每聊天最多 ${MAX_SCHEDULES_PER_CHAT} 条）。`) : [T("当前定时消息："), ...mine.map(describe)].join("\n"));
        return { handled: true };
      }
...
init_store_file();
import { appendFile as appendFile2, mkdir as mkdir4, readFile as readFile5, writeFile as writeFile3 } from "node:fs/promises";
...
var RefIndex = class {
  ...
  constructor({ logger, appId, max = DEFAULT_MAX }) {
    ...
    this.#path = join3(pluginDataDir(), `ref-index-${safeAppId}.jsonl`);
  }
  /** 落盘路径（设置界面展示用）。 */
  get path() {
    return this.#path;
  ...
  #persist(entry) {
    const task = this.#queue.then(async () => {
      try {
        await mkdir4(pluginDataDir(), { recursive: true });
...
`, "utf8");
  ...
  async #compact() {
    ...
    try {
      ...
      await writeFile3(this.#path, text ? `${text}
` : "", "utf8");
    } catch (error) {
...
function installReplyPump(ctx, { bots, outbox, quota, logger }) {
  ...
  const send = async (bot, sessionId, rawText) => {
    ...
    const quoteReply = record.target.scope === "group" && config.quoteReply !== "off" && (config.quoteReply === "all" || record.quoteMention) && Boolean(record.selfIdx);
    ...
    for (const [index, chunk] of chunks.entries()) {
      ...
      const sendOnce = (withQuote) => bot.client.sendReply(record.target, chunk, withQuote ? { markdown: usedMarkdown, quoteMsgId: record.selfIdx } : { msgId: record.msgId || void 0, msgSeq: seq, markdown: usedMarkdown });
      try {
...
function filePath(appId) {
  return join4(pluginDataDir(), "self-openids", `${appId.replace(/[^A-Za-z0-9_-]/g, "_")}.json`);
}
var SelfOpenidStore = class {
  ...
  async load(appId) {
    const map = /* @__PURE__ */ new Map();
    try {
      const raw = JSON.parse(await readFile6(filePath(appId), "utf8"));
  ...
  async save(appId, map) {
    try {
      await mkdir5(join4(pluginDataDir(), "self-openids"), { recursive: true });
      await writeFile4(filePath(appId), JSON.stringify(Object.fromEntries(map), null, 2), "utf8");
    } catch (error) {
...
function statsPath(appId) {
  return join5(pluginDataDir(), "stats", `${appId.replace(/[^A-Za-z0-9_-]/g, "_")}.json`);
}
var StatsStore = class {
  ...
  async load(appId) {
    ...
    try {
      const raw = JSON.parse(await readFile7(statsPath(appId), "utf8"));
  ...
  async save(appId, counters) {
    try {
      await mkdir6(join5(pluginDataDir(), "stats"), { recursive: true });
      await writeFile5(
        statsPath(appId),
...
// src/host/memory.ts
init_store_file();
import { mkdir as mkdir7, readFile as readFile8, rm as rm3, writeFile as writeFile6 } from "node:fs/promises";
...
function memoryPath(chatKey) {
  return join6(pluginDataDir(), "memory", `${safeName(chatKey)}.md`);
}
...
var ChatMemoryStore = class {
  ...
  async #loadMarkdown(chatKey) {
    let raw;
    try {
      raw = await readFile8(memoryPath(chatKey), "utf8");
    } catch {
      return null;
    }
    const entries = [];
  ...
  async clear(chatKey) {
    ...
    try {
      await rm3(memoryPath(chatKey), { force: true });
    } catch (error) {
  ...
  async #save(chatKey, entries) {
    try {
      await mkdir7(join6(pluginDataDir(), "memory"), { recursive: true });
      const lines = ["# QQ 聊天长期记忆", ""];
      for (const e of entries) lines.push(`- ${e.text}`);
      await writeFile6(memoryPath(chatKey), `${lines.join("\n")}
`, "utf8");
      this.#cache.set(chatKey, entries);
    } catch (error) {
...
var OUTBOX_PATH = () => join7(pluginDataDir(), "outbox.json");
var OUTBOX_CAP = 200;
var MAX_ATTEMPTS = 5;
var Outbox = class {
  #items = [];
...
var QUOTA_PATH = () => join8(pluginDataDir(), "quota.json");
