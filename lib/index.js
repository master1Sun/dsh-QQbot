...
function registerRpcChannel({ ctx, fence, dispatch, onSseOpen }) {
...
// src/host/admin/sse-hub.ts
var SSE_EVENT_BOTS_CHANGED = "bots-changed";
var HEARTBEAT_INTERVAL_MS = 25e3;
function createSseHub({ logger }) {
...
function contentDigest(text) {
  return createHash2("sha1").update((text ?? "").replace(/\s+/g, " ").trim()).digest("hex").slice(0, 16);
}
