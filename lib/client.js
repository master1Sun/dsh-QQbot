...
var EN = Object.freeze({
  ...
  "回复以原生引用气泡指向触发消息（点击可跳转定位到被引用的那句话）；仅主动消息（如定时任务回复）无法原生引用，回退为文本引用（Markdown 引用块 /「昵称：原话」）。off=不引用；at=仅 @/单聊（避免群全量刷屏，推荐）；all=全部回复都引用。此外，用户引用聊天里某条消息时，被引用的原文会始终注入模型上下文，让它知道对方在回应什么。": "Replies point at the triggering message with a native quote bubble (click it to jump to the quoted message). Only proactive messages (e.g. scheduled-task replies) cannot carry a native reference and fall back to a text quote (Markdown blockquote / “nickname: message”). off = no quote; at = @-mentions & DMs only (avoids group flooding, recommended); all = every reply. Also, when a user quotes another message in chat, the quoted text is always injected into the model's context so it knows what the user is responding to.",
  ...
  "文本引用最多显示多少字（仅主动消息回退为文本引用时使用；原生引用气泡由 QQ 客户端自行截断），超出部分以省略号结尾。": "How many characters of the quoted text to show at most (used only when a proactive message falls back to a text quote; the native quote bubble is truncated by the QQ client itself). Longer text ends with an ellipsis.",
...
function SettingRow(props) {
  return h(
    "div",
    { className: "qbot-settingRow", key: props.rowKey },
    h(
      "div",
...
function QqbotSettingsTab({ rpcCall }) {
  ...
  const sectionCard = (title, desc, body, action) => h(
    "section",
    { className: "qbot-section" },
    h(
      "div",
  ...
  const detailView = h(
    ...
    h(
      "section",
      { className: "qbot-section is-danger" },
      h(
        "div",
        { className: "qbot-sectionHead" },
        h(
    ...
    picker ? h(
      ...
      h(
        ...
        h(
          ...
          picker.loading ? h("div", { className: "qbot-modalState" }, h("span", { className: "qbot-spinner", "aria-hidden": "true" }), "正在读取目录…") : picker.error ? h("div", { className: "qbot-modalState qbot-modalError" }, picker.error) : [
            picker.parent ? h(
              "button",
            ...
            picker.dirs.length === 0 ? h("div", { className: "qbot-modalState" }, "该目录下没有子文件夹") : null
          ]
        ),
        h(
          "div",
