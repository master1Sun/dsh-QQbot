...
var EN = Object.freeze({
  ...
  "内容": "Content",
  "给 AI 的任务指令（如「总结昨天群聊的重点」「价格低于 100 再提醒我」）。到点 AI 会自己调用工具取数、加工，再决定发什么；若判断无事可报会自动静默——不打扰大家，也不占主动消息配额。": `A task instruction for the AI (e.g. "summarize yesterday's chat highlights", "alert me only if the price drops below 100"). At send time the AI fetches data with tools, processes it, and decides what to send; if there is nothing worth reporting it stays silent — no noise, and no proactive-message quota consumed.`,
  ...
  "支持 daily / interval / cron / at 四种触发条件，以及 文本 / AI 智能任务 / 执行命令 三种执行方式。": "Four triggers — daily / interval / cron / at — and three actions: text / AI smart task / run a command.",
  "待补算": "TBD",
  ...
  // 编辑表单 · ③ 到点做什么
  "③ 到点做什么": "③ What it does",
  "选择执行方式；除「直接发送文本」外，都能在「执行」与「发送」之间插入加工与判断。": "Pick an action. Except for plain text sending, every action can insert processing and a decision between “run” and “send”.",
  "执行方式": "Action",
  "文本=到点原样发送；AI 智能任务=把内容当任务指令，到点 AI 自己取数、加工、决定发不发；执行命令=确定性跑一条命令，再按加工指令与发送门控推送给用户。": "text = send as-is; AI smart task = treat the content as a task instruction and let the AI fetch data, process it, and decide whether to send; run command = deterministically run a command, then push it through the processing instruction and send gate.",
  ...
  "raw = 直接把命令输出推送给用户；ai = 先按「数据加工指令」把输出整理后推送（输出很长或含噪音时推荐）。": "raw = push the command output as-is; ai = process it according to the “data processing instruction” first (recommended when output is long or noisy).",
  ...
  "规定把命令输出处理成什么样再发：筛选、排序、限行、固定格式都写在这里。留空则用内置的「整理成一段简洁播报」要求。": "Describe how the command output should be shaped before sending: filtering, sorting, row limits, fixed formats. Leave empty to use the built-in “condense into a short report” requirement.",
  "例如：只保留今天新增的订单，按金额从高到低排列，最多 5 条；没有新增就什么都别发": "e.g. Keep only orders created today, sorted by amount descending, at most 5; send nothing if there are none",
  "数据加工指令": "Data processing instruction",
  "任务目标（可选）": "Task goal (optional)",
  "一句话说明这条任务服务于什么判断，供 AI 分诊时理解意图。": "One line on what decision this task serves, so the AI understands intent when triaging.",
  "例如：盯住竞品价格波动": "e.g. watch a competitor's price moves",
  "任务目标": "Task goal",
  "通知条件（可选）": "Notify condition (optional)",
  "用自然语言写明「什么时候才值得打扰大家」。不满足时本次静默不发，也不占主动消息配额。": "State in plain language when it is worth interrupting people. When unmet, nothing is sent this round and no proactive-message quota is consumed.",
  ...
  "开启后，投递前再复核一次草稿是否满足上面的目标与通知条件，不达标就不发。tool 模式为独立模型二次复核，ai 模式为强化自查。": "When on, the draft is reviewed once more against the goal and notify condition before delivery; if it falls short, nothing is sent. For tool mode this is a second, independent model review; for ai mode it is a strengthened self-check.",
  "关闭校验": "Off",
  "开启校验": "On",
  "条件触发": "Conditional",
  "发送门控": "Send gate",
  "投递到 QQ 前的最后一道判断：changed 适合「有变化才播报」，nonempty 适合「有异常才报警」。被拦下时不投递，也不消耗主动消息配额。": "The final check before delivering to QQ: changed suits “report only on change”, nonempty suits “alert only on output”. When blocked, nothing is delivered and no proactive-message quota is consumed.",
...
function ScheduleDialog(props) {
  ...
  const actionField = (e) => {
    ...
    const contentRow = editRow(
      "内容",
      e.mode === "ai" ? "给 AI 的任务指令（如「总结昨天群聊的重点」「价格低于 100 再提醒我」）。到点 AI 会自己调用工具取数、加工，再决定发什么；若判断无事可报会自动静默——不打扰大家，也不占主动消息配额。" : "到点直接发送的文本，上限 2000 字。",
      TextArea({
        rows: 3,
    ...
    h(
                  ...
                  (e) => h(
                    ...
                    h(
                      ...
                      h(
                        ...
                        (e.mode === "ai" || e.mode === "tool") && e.notifyWhen ? h("span", { className: "qbot-chip" }, "条件触发") : null,
                        (e.mode === "ai" || e.mode === "tool") && e.verify === true ? h("span", { className: "qbot-chip" }, "发送前自校验") : null,
      ...
      !ed ? h(
        ...
        h(
          "span",
          { className: "qbot-hint" },
          scheduleModal.maxPerChat > 0 ? scheduleModal.botScope === "all" ? `所有机器人共 ${scheduleModal.items.length} 条（每个群/单聊最多 ${scheduleModal.maxPerChat} 条）` : `共 ${scheduleModal.items.length} 条（每个群/单聊最多 ${scheduleModal.maxPerChat} 条）` : scheduleModal.botScope === "all" ? `所有机器人共 ${scheduleModal.items.length} 条` : `共 ${scheduleModal.items.length} 条`
        ),
        h(
          "div",
          { className: "qbot-viewActions" },
