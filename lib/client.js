...
function ScheduleManager(props) {
  ...
  const toggleSchedule = async (id, currentlyEnabled) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSchedNotice(
          currentlyEnabled ? { ok: true, text: t("sched.disabledToast") } : nextAt ? { ok: true, text: fmt("sched.enabledToast", formatTime(nextAt)) } : { ok: false, text: t("sched.enabledNoNext") }
        );
  ...
  const ROOT = { display: "flex", flexDirection: "column", height: "100%", minHeight: 0, boxSizing: "border-box" };
  return h(
    "div",
...
var botsListeners = /* @__PURE__ */ new Set();
function onBotsChanged(fn) {
...
var PANEL_VISIBLE_KEY = "dsh-qqbot.panel.visible";
...
function setPanelVisible(v) {
  try {
    localStorage.setItem(PANEL_VISIBLE_KEY, v ? "1" : "0");
  } catch {
  }
  notifyBotsChanged();
}
...
// src/client/right-panel/TitleView.tsx
...
function TitleView() {
  useLocale();
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { style: wrap, children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(QqBotGlyph, { uid: "panel-chip", size: 15, className: "qbot-panelMark" }),
    t("panel.title")
  ] });
}
...
var inject = ["slots", "connection", "locale", "sidebarRightTabs", "sidebarRight"];
...
var QQBOT_PANEL_ID = "@sunjuntao/dsh-qqbot";
var QQBOT_PANEL_KIND = "qqbot-sched";
...
function QqbotSettingsTab({ rpcCall }) {
  ...
  const [panelVisible, setPanelVisibleState] = React10.useState(() => getPanelVisible());
  ...
  const listView = h(
    ...
    h(
      ...
      h(
        "div",
        { className: "qbot-surfaceBody" },
        h(
          "label",
          { className: "qbot-switchRow" },
          h(
            "span",
            { className: "qbot-switchText" },
            h("strong", null, t("ui.panelSwitchTitle")),
            h(
              "p",
              { style: { margin: "2px 0 0", fontSize: "12px", color: "var(--dsw-alias-label-tertiary, #8f959e)" } },
...
function apply(ctx) {
  ...
  let disposePanelTab = null;
  const registerPanelTab = () => {
    if (disposePanelTab) return;
    disposePanelTab = ctx.sidebarRightTabs.register({
      id: QQBOT_PANEL_ID,
  ...
  const scheduleWsReconnect = () => {
    ...
    const delay = Math.min(1e3 * 2 ** wsRetry, 1e4);
