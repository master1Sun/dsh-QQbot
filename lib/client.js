...
var EN = Object.freeze({
  ...
  "该群非 @ 全量消息使用的 Agent Preset（只聊天不执行工具）。留空跟随机器人配置。": "Agent Preset for non-@ full group messages in this group (chat only, no tools). Leave empty to follow the bot config.",
...
function OverrideDialog(props) {
  ...
  const [draft, setDraft] = React5.useState(() => buildDraft(overrides, editOpenid));
        ...
        h(
          ...
          editRow(
            ...
            TextInput({
              ...
              onChange: (ev) => setOverrideField("openid", ev.target.value),
              "aria-label": "群 openid"
            })
          ),
          editRow(
          ...
          editRow(
            ...
            h(
              ...
              {
                className: "qbot-settingSelect",
                value: String(draft.memoryEnabled),
                onChange: (ev) => setOverrideField("memoryEnabled", ev.target.value),
                "aria-label": "长期记忆"
              },
              h("option", { value: "" }, "跟随默认"),
              h("option", { value: "on" }, "启用"),
          ...
          editRow(
            ...
            h(
              ...
              {
                className: "qbot-settingSelect",
                value: String(draft.agentPresetChat),
                onChange: (ev) => setOverrideField("agentPresetChat", ev.target.value),
                "aria-label": "聊天 Preset"
              },
              h("option", { value: "" }, "跟随默认"),
              presetOptions(presets).map((o) => h("option", { key: o.value, value: o.value }, o.label))
            )
          ),
          editRow(
            "敏感词列表",
...
function ScheduleDialog(props) {
  const { rpcCall, detailAppId, onClose } = props;
  const [scheduleModal, setScheduleModal] = React6.useState({
    loading: true,
    error: "",
    items: [],
    botScope: "current",
  ...
  const [scheduleRemoving, setScheduleRemoving] = React6.useState("");
  const [scheduleToggling, setScheduleToggling] = React6.useState("");
  const [scheduleTesting, setScheduleTesting] = React6.useState("");
  const [schedNotice, setSchedNotice] = React6.useState(null);
  const [cronPreview, setCronPreview] = React6.useState(null);
...
function WorkspacePickerDialog(props) {
  ...
  const [picker, setPicker] = React7.useState({
    path: "",
    parent: null,
    dirs: [],
    selected: "",
...
function QqbotSettingsTab({ rpcCall }) {
  ...
  const [bots, setBots] = React8.useState(null);
  const [catalogs, setCatalogs] = React8.useState(EMPTY_CATALOGS);
  const [form, setForm] = React8.useState({});
  const [page, setPage] = React8.useState("list");
  const [detailAppId, setDetailAppId] = React8.useState("");
  const [scheduleOpen, setScheduleOpen] = React8.useState(false);
  const [archiveOpen, setArchiveOpen] = React8.useState(false);
  const [pickerOpen, setPickerOpen] = React8.useState(false);
  const [overrideEdit, setOverrideEdit] = React8.useState(null);
  ...
  const [reconnecting, setReconnecting] = React8.useState(false);
  const [update, setUpdate] = React8.useState({
    busy: false,
    message: "",
    done: false
  });
  const refresh = React8.useCallback(async (appId) => {
    try {
  ...
  const [resetting, setResetting] = React8.useState(false);
  ...
  const detailBot = React8.useMemo(
