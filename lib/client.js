...
function getPanelVisible() {
  try {
    return localStorage.getItem(PANEL_VISIBLE_KEY) !== "0";
  } catch {
    return true;
  }
}
...
function QqbotSettingsTab({ rpcCall }) {
  ...
  const [update, setUpdate] = React10.useState({
    busy: false,
    message: "",
    done: false
  });
...
function apply(ctx) {
  ...
  const syncPanelTab = async () => {
    try {
      ...
      const bots = res.ok ? val(res)?.bots : void 0;
      const anyConnected = Array.isArray(bots) && bots.some((b) => b?.ws?.state === "connected");
  ...
  const panelTabTimer = setInterval(() => void syncPanelTab(), 15e3);
  ctx.effect(
    () => () => {
